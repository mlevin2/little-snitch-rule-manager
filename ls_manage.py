...
def update_rule(args):
    ...
    if cr_key:
        ...
        # Issue fix: mosh-server was previously set as "trustedAnchor" with a SHA256 hash, which is invalid.
        ...
        print(f"No existing code requirement found. Creating new 'fileHash' requirement for {resolved_path}")
        # Ensure codeRequirements dict exists
        if "codeRequirements" not in config:
            config["codeRequirements"] = {}
            
        config["codeRequirements"][resolved_path] = {
            "type": "fileHash",
            "codeIdentifier": new_hash
        }
...
def main():
    ...
    update_parser.add_argument("--protocol", default="udp", choices=["udp", "tcp"], help="Protocol (udp/tcp)")
    update_parser.add_argument("--direction", default="both", choices=["incoming", "outgoing", "both"], help="Direction")
    update_parser.add_argument("--remote", default="any", help="Remote scope (any, local-net, etc)")
    update_parser.add_argument("--replace", action="store_true", help="Replace existing rules for this binary")
